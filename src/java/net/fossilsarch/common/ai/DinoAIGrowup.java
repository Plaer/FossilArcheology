...
public class DinoAIGrowup extends EntityAIBase {
	...
	public void updateTask() {
		...
		AITarget.CheckSkin();
		if (AITarget.getHealth()<AITarget.getMaxHealth()) AITarget.heal(healingOnGrowing);
	...
	if (!AITarget.CheckSpace()){	
		...
		if (AITarget.getHealth()>healingOnGrowing) AITarget.damageDinosaur(DamageSource.generic, healingOnGrowing);
		AITarget.updateSize(false);
		AITarget.setPosition(AITarget.posX,AITarget.posY,AITarget.posZ);
