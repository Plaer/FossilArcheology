...
public abstract class EntityDinosaurce extends EntityTameable implements IEntityAdditionalSpawnData{
    ...
    public void damageDinosaur(DamageSource damageSource, float amount)
    {
    	this.damageEntity(damageSource, amount);
    }
		...
		if (this.getDinoAge()>4){
			...
			if (this.BreedTick==0){
				...
				if (new Random().nextInt(100)<GroupAmount){
					...
					if(	worldObj.checkNoEntityCollision(entityegg.boundingBox) && worldObj.getCollidingBoundingBoxes(entityegg, entityegg.boundingBox).size() == 0) worldObj.spawnEntityInWorld(entityegg);
