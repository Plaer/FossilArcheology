...
{	
        	...
        	for (ScanZ=0;ScanZ<WidthZ;ScanZ++){
        		for (ScanX=0;ScanX<WidthX;ScanX++){
        			...
        			if (TmpBlock!=0){
            			...
            			world.setBlock(chunkX-WidthX/2+ScanX, posY+ScanY, chunkZ-WidthZ/2+ScanZ, TmpBlock,TmpMD, 2);
        				...
        				if (MatCheck!=Material.water && MatCheck!=Material.sand){
        					world.setBlock(chunkX-WidthX/2+ScanX, posY+ScanY, chunkZ-WidthZ/2+ScanZ, 0,0, 3);
        				}
        			}
        		}
        	}
    ...
    public ItemStack GetRandomContent(Random rand,boolean isDisc){
		...
		if (isDisc) return (new ItemStack(Item.itemsList[Item.record13.itemID+rand.nextInt(11)],1));
		else{
